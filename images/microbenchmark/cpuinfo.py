...
import sys, os, glob, errno, struct, collections
...
def findCpuinfo(paths=[INITCPUINFO, "/proc/cpuinfo"], needCPUs=None):
    ...
    for path in paths:
        ...
        sys.stderr.write("No cpuinfo found (tried %s)" % ":".join(paths))
        sys.exit(1)

    if needCPUs is None:
        needCPUs = getCPUSet("present")
    if needCPUs.difference(set(int(cpu["processor"]) for cpu in cpuinfo)):
        sys.stderr.write(sys.stderr, ("Warning: Some processors are missing from %s.  "
                              "Are all processors online?" % path))
        if not os.path.exists(INITCPUINFO):
            sys.stderr.write(("Protip:  Add `cp /proc/cpuinfo %s' to "
